...
![Панель FastAction под челкой: играет музыка](docs/screenshots/ru/panel-top.png)
...
![Вкладки слева, как было всегда](docs/screenshots/ru/panel-left.png)
...
![Плеер в панели](docs/screenshots/ru/music.png)
...
Когда в письме приходит код — он сам оказывается в челке: крупно, с кнопкой «Копировать» и крестиком.
После копирования плашка закрывается. Висит она дольше обычного уведомления, 10 секунд,
и это время настраивается.

Число становится кодом, только если рядом стоит слово вроде «код», «подтверждения» или `verification`.
Номера заказов, телефоны, суммы, даты, годы, индексы и номера карт кодом не считаются —
в самотесте на это 27 случаев, половина из которых проверяет, что мы **не** срабатываем.

Коды из **iMessage и SMS** тоже находятся, если включить это в **Настройки → Уведомления**.
Для чтения базы Сообщений macOS попросит полный доступ к диску; читаются только новые входящие
и только ради кода — переписка никуда не сохраняется и нигде не показывается.
...
- Сообщения читаются только при включённом поиске кодов: только новые входящие, только ради кода,
...
Форма панели вдохновлена [Cyclop](https://github.com/akalikbergenov/cyclop).
